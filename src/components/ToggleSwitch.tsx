...
export default function ToggleSwitch() {
  ...
  return (
    <label className="relative inline-flex items-center cursor-pointer">
      <input type="checkbox" name='toleranceWindow' value='on' className="sr-only peer" checked={toggled} onChange={handleChange} />
      ...
      <span className="ms-3 text-sm">Toggle {toggled ? 'ON' : 'OFF'}</span>
    </label>
  );
};
