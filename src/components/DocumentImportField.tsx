    ...
    <div className='flex flex-col gap-4'>
      <div className='flex flex-col items-center border rounded-lg p-4 gap-4'>
        <div
          {...getRootProps({
            className:
              'flex flex-col items-center justify-center h-[118px] w-full border-2 border-dashed rounded-lg gap-5 cursor-pointer',
        ...
        >
          ...
          {!isDragActive ? (
            <p className='text-xs'>
              Drag & Drop Here Or{' '}
              <span className='text-blue-900 font-bold hover:underline'>Browse</span>
            </p>
          ) : (
            <p className='text-xs'>Drop File</p>
          )}
          <input
            {...getInputProps({ type: 'file', className: 'hidden' })}
            ref={inputRef}
            {...field}
          />
        </div>
